  ...
  try {
    ...
    for (const coords of lines) {
      if (Array.isArray(coords) && coords.length > 1) {
        // GeoJSON usa [lng, lat]; turf.lineString espera ese orden
...
export const createRoadSelectionManager = (map, opts = {}) => {
  ...
  return { register, clearSelection, selectByKey, selectByFeature, _debugIndex: index };
};

/* ===================== onEachFeature con click amplio ===================== */

/**
 * Configura eventos por feature; deja el click ampliado,
 * registra la capa en el selectionManager y notifica la selección del grupo.
 */
export const onEachRoadFeature = (feature, layer, options = {}) => {
  const selectionManager = options.selectionManager || null;

  // Registrar para agrupación
  if (selectionManager && typeof selectionManager.register === 'function') {
    selectionManager.register(feature, layer);
  }
  ...
  // Hover feedback del tramo
  layer.on({
    mouseover: (e) => {
      const target = e.target;
      target.setStyle({ weight: (originalWeight ?? 2) + 2, opacity: 1, color: '#1e40af' });
    },
    mouseout: (e) => {
      const target = e.target;
      target.setStyle({ weight: (originalWeight ?? 2), opacity: 0.6, color: '#333333' });
    }
  });
};
