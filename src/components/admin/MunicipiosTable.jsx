...
const MunicipiosTable = ({ zoneStates, onZoneStateChange, onBulkUpdate, onRefresh }) => {
  const { isAdmin, canEditMunicipio, getAccessibleMunicipios } = useRole();
  const [selectedZones, setSelectedZones] = useState([]);
  const [bulkState, setBulkState] = useState('green');

  const accessibleZones = Object.keys(zoneStates).filter(zoneName =>
    getAccessibleMunicipios(Object.keys(zoneStates)).includes(zoneName)
  );
  ...
  const handleSelectAll = () => {
    if (selectedZones.length === accessibleZones.length) setSelectedZones([]);
    else setSelectedZones([...accessibleZones]);
  };

  const handleBulkUpdate = () => {
    if (selectedZones.length === 0) return;
    const updates = selectedZones.map(zoneName => ({ zone_name: zoneName, state: bulkState }));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={onRefresh} className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">
          Actualizar
        </button>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Municipio/Zona</th>
            ...
            {accessibleZones.map((zoneName) => {
              ...
              return (
                <tr key={zoneName} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{zoneName}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(zone?.updated_at)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{zone?.updated_by || 'Sistema'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    {canEdit ? (
                      <div className="flex space-x-2">
                        <button onClick={() => onZoneStateChange(zoneName, 'green')} className="text-green-600 hover:text-green-900" title="Habilitar">✓</button>
                        <button onClick={() => onZoneStateChange(zoneName, 'yellow')} className="text-yellow-600 hover:text-yellow-900" title="Alerta">⚠</button>
                        <button onClick={() => onZoneStateChange(zoneName, 'red')} className="text-red-600 hover:text-red-900" title="Suspender">✕</button>
                      </div>
                    ) : (
      ...
      {accessibleZones.length === 0 && (
        <div className="text-center py-8 text-gray-500">No hay zonas disponibles para mostrar.</div>
      )}
    </div>
  );
};
